...
function ToggleClothing(whic, extra)
	...
	if type(whic) == 'table' then
		which = whic.id or (whic.arg and whic.arg.id)
	end
	Wait(50)
	...
	if not Toggle then
		QBCore.Functions.Notify(Lang:t('error.invalid_clothing_item'), 'error')
		return false
	end
	...
	if which ~= 'Mask' then
		if not Gender then
			QBCore.Functions.Notify(Lang:t('info.wrong_ped'), 'error')
			return false
		end
	end
	...
	if not Toggle.Table.Standalone then
		...
		QBCore.Functions.Notify(Lang:t('info.no_variants'), 'error')
		return false
	else
		if not LastEquipped[which] then
	...
	QBCore.Functions.Notify(Lang:t('info.already_wearing'), 'error')
	return false
end
...
RegisterNetEvent('qb-radialmenu:client:toggleClothing', function(data)
	ToggleClothing(data)
end)
...
function ToggleProps(whic)
	local which = whic
	if type(whic) == 'table' then
		which = whic.arg or whic -- Check for both formats
	end
	Wait(50)
	...
	if not Prop then
		Notify(Lang:t('error.invalid_clothing_item'), 'error')
		return false
	end
